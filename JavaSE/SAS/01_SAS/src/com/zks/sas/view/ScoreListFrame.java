...
public class ScoreListFrame extends JFrame {
    ...
    public ScoreListFrame() {
        ...
        JLabel lblNewLabel = new JLabel("成绩信息列表");
        ...
        btnUpdate.addActionListener(e -> {
            ...
            int  scoreId = Integer.parseInt(table.getValueAt(row, 0).toString());
            ScoreUpdateFrame frame = new ScoreUpdateFrame(scoreId);
        ...
        btnDelete.addActionListener(e -> {
            ...
            int r = JOptionPane.showConfirmDialog(this, "您要删除编号为[" + clsId + "]的成绩吗?", "提示",
                    JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
            if (r == JOptionPane.YES_OPTION) {
                IClassInfoDAO classInfoDAO = new ClassInfoDAOImpl();
                if (classInfoDAO.deleteById(clsId)) {
                    loadData();
                    JOptionPane.showMessageDialog(null, "删除成绩信息成功!", "成功", JOptionPane.INFORMATION_MESSAGE);
                } else {
                    JOptionPane.showMessageDialog(null, "删除成绩信息失败!", "失败", JOptionPane.ERROR_MESSAGE);
                }
            }
        });
        loadData();
    ...
    private void loadData(){
        ...
        //总行数 数据库
        int rowCount = scoreDAO.getRowCount();
        //总页数 总行数 与 pageSize计算而来
        pageCount = scoreDAO.getPageCount(pageSize);
        //当前页具体数据

        List<Score> scores = scoreDAO.getNowPageData(pageSize, nowPage);
        ...
        for (Score sco : scores) {
            int scoreId = sco.getScoreId();
            int lesId = sco.getLesId();
            String stuNum = sco.getStuNum();
            int score = sco.getScore();
            Object[] row = {scoreId,lesId,stuNum,score};
            model.addRow(row);
        }
